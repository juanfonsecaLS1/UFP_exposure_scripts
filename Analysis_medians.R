...
sum_inhvol<-Sum_overall%>%group_by(Participan,TypeLeg,TypeSpeed)%>%summarise(mean_inhvol=sum(sec_inhvol,na.rm = TRUE),
...
sum_ps<-Sum_overall%>%group_by(Participan,TypeLeg,TypeSpeed)%>%summarise(mean_dose_ps=mean(sec_dose,na.rm = TRUE),
...
plot_cumdose<-ggplot(Sum_overall,aes(col=TypeSpeed))+
  geom_line(aes(x=cum_dist,y=cum_dose*10^(-6),linetype="Dynamic"),size=0.7)+
